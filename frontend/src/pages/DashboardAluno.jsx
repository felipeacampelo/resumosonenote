...
import { 
  BookOpen, Clock, Target, FileText, TrendingUp, Calendar,
  Flame, Award, BarChart3, ChevronRight, ArrowRight, Filter
} from 'lucide-react';
...
const DashboardAluno = () => {
  ...
  const [filtroTempo, setFiltroTempo] = useState('total'); // 7dias, 30dias, total
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-purple-200 mt-1">
                {plano?.nome ? `Estudando para ${plano.nome}` : 'Escolha um plano para começar'}
              </p>
            </div>
            {plano?.id && (
              <button
                onClick={() => navigate(`/meus-estudos/${plano.id}`)}
                className="flex items-center gap-2 px-4 py-2 bg-white text-purple-700 rounded-lg hover:bg-purple-50 transition font-medium"
              >
                Continuar Estudando
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            ...
            <div className="bg-white/10 backdrop-blur rounded-xl p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold">{sequencia.streak_atual}</p>
            ...
            <div className="bg-white/10 backdrop-blur rounded-xl p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold">{estatisticas_gerais.percentual_concluido}%</p>
            ...
            <div className="bg-white/10 backdrop-blur rounded-xl p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold">{estatisticas_gerais.tempo_total_horas}h</p>
            ...
            <div className="bg-white/10 backdrop-blur rounded-xl p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold">{estatisticas_gerais.percentual_acerto}%</p>
      ...
      <main className="ml-64 px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="text-center p-4 bg-gray-50 rounded-lg">
                  <p className="text-3xl font-bold text-purple-600">
                    {estatisticas_gerais.assuntos_estudados}
                  </p>
                  <p className="text-sm text-gray-600">Assuntos Estudados</p>
                  <p className="text-xs text-gray-400">de {estatisticas_gerais.total_assuntos}</p>
                </div>
                
                <div className="text-center p-4 bg-gray-50 rounded-lg">
                  <p className="text-3xl font-bold text-blue-600">
                    {sequencia.total_dias_estudados}
                  </p>
                ...
                <div className="text-center p-4 bg-gray-50 rounded-lg">
                  <p className="text-3xl font-bold text-green-600">
                    {estatisticas_gerais.questoes_feitas}
                  </p>
                  <p className="text-sm text-gray-600">Questões Feitas</p>
                  <p className="text-xs text-gray-400">{estatisticas_gerais.questoes_acertadas} acertadas</p>
                </div>
                
                <div className="text-center p-4 bg-gray-50 rounded-lg">
                  <p className="text-3xl font-bold text-orange-600">
                    {sequencia.streak_maximo}
                  </p>
                ...
                <div className="flex gap-6">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-600">
                      <strong>{Math.round(sequencia.media_tempo_diario)}</strong> min/dia
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <FileText className="h-4 w-4 text-gray-400" />
                    <span className="text-sm text-gray-600">
                      <strong>{Math.round(sequencia.media_questoes_diario)}</strong> questões/dia
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              {por_disciplina && por_disciplina.length > 0 ? (
                ...
                <p className="text-gray-500 text-center py-8">
                  Comece a estudar para ver seu progresso por disciplina
                </p>
              )}
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-gray-900">Estudados Recentemente</h2>
              </div>
              ...
              {plano?.id && (
                <button
                  onClick={() => navigate(`/meus-estudos/${plano.id}`)}
                  className="w-full mt-4 flex items-center justify-center gap-2 px-4 py-2 text-purple-600 hover:bg-purple-50 rounded-lg transition text-sm font-medium"
                >
